...
u32 XFinn_feeder_chiplet_Get_predicted_index_vld(XFinn_feeder_chiplet *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XFinn_feeder_chiplet_ReadReg(InstancePtr->Control_BaseAddress, XFINN_FEEDER_CHIPLET_CONTROL_ADDR_PREDICTED_INDEX_CTRL);
    return Data & 0x1;
}
